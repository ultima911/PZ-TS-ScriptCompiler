...
import { Logger } from '../../../lib/common/util/Logger';
...
export abstract class Compiler {
    public static compile(pzModule: PZModule, stringBuilder?: StringBuilder) {
        const sb = stringBuilder ?? new StringBuilder(true);
    ...
    private static buildImports(pzModule: PZModule, sb: StringBuilder) {
        ...
        pzModule.imports.forEach((pzImport: PZModule) => {
            sb.appendLine(pzImport.id);
        });
